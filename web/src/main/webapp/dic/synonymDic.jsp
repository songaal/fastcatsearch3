...
<%
	...
	String dictionaryId = category + "SynonymDictionary";
	...
	SetDictionary synonymDictionary = dbHandler.getDAO(dictionaryId);
	...
	if(!"".equals(keyword)) {
		result = synonymDictionary.selectPageWithKeyword(keyword,startRow,pageSize);
		recordCount = synonymDictionary.selectCountWithKeyword(keyword);
	}else{
		keyword = "";
		result = synonymDictionary.selectPage(startRow,pageSize);
		recordCount = synonymDictionary.selectCount();
	}
%>
...
function clearSynonymDic(){
	if(confirm("모든 단어를 삭제하시겠습니까?")){
		submitPost('dbservice.jsp', {cmd: 20, category:'<%=category %>', dic: 'synonymDic' });
	}
}
...
<div id="sidebar">
	...
	<div class="sidebox">
		<h3>도움말</h3>
			<ul class="latest">
			<li>유사어는 대표단어를 여러단어로 확장검색하여 검색결과를 풍부하게 해준다.</li>
			<li>새로추가버튼을 눌러 대표단어를 추가하고 오른쪽 영역에서 하위단어를 추가한뒤 저장버튼을 눌러 최종적으로 유사어를 저장한다.</li>
			<li>파일로추가 : UTF-8로 인코딩의 텍스트파일을 선택하여 단어들을 일괄입력할 수 있다. 엔트리형식은 [대표단어]:[단어1],[단어2],[단어3].. 과 같다.</li>
...
<div id="mainContent">
	...
	<div id="conLbox.del">
		<div class="searchBox">
		<div class="box">
		<form action="synonymDic.jsp" method="post">
...
<%
			...
			for(int i = 0; i < result.size(); i++){
				...
				String[] synonymList = vo.keyword.split(",");
				int count = synonymList.length;
				String representative = "";
				StringBuilder sb = new StringBuilder();
				for(String synonym : synonymList){
					
					if(synonym.startsWith("@")){
						representative = synonym.substring(1);
					}else{
						if(sb.length() > 0){
							sb.append(",");
						}
...
%>
		...
		<div class="list_bottom">
			...
			<div style="width:100%">
				<span style="float:left">
					<input type="button" value="선택삭제" onclick="javascript:deleteSynonymWord();" class="btn_del" style="float:left"/>
					<input type="button" value="초기화" class="btn_init" onclick="clearSynonymDic();" style="float:left"/>
				</span>
				<p class="addbox">
					<input type="text" class="inp02" id="synonymKey" tabindex="1" size="15" />
					<input type="text" class="inp02" id="synonymValue" tabindex="2" size="40" onkeydown="javascript:addSynonymWord(event);"/>
					<input type="button" value="새로추가" class="btn_c" onclick="addSynonymWord();"/>
				</p>
				<p class="clear"></p>
			</div>
	...
	<form action="dbservice.jsp" method="post" id="addwordForm">
		<input type="hidden" name="synonymWord" id="synonymWord"/>
		<input type="hidden" name="category" value="<%=category %>"/>
