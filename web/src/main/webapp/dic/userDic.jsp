...
<%
	...
	String dictionaryId = category + "UserDictionary";
	int pageSize = 30;
	...
	SetDictionary userDictionary = dbHandler.getDAO(dictionaryId);
	...
	if(!"".equals(keyword)){
		result = userDictionary.selectPageWithKeyword(keyword,startRow,pageSize);
		recordCount = userDictionary.selectCountWithKeyword(keyword);
	}else{
		keyword = "";
		result = userDictionary.selectPage(startRow,pageSize);	
		recordCount = userDictionary.selectCount();
	}
%>
...
function clearUserDict(){
	if(confirm("모든 단어를 삭제하시겠습니까?")){
		submitPost('dbservice.jsp', {cmd: 20, category:'<%=category %>', dic: 'userDic' });
	}
}
...
<div id="sidebar">
	...
	<div class="sidebox">
		<h3>도움말</h3>
			<ul class="latest">
			<li>신조어 또는 미등록어는 사용자단어 사전에 추가하면 검색이 가능하다.</li>
			<li>새로추가버튼을 눌러 사용자단어를 추가한다.</li>
...
<h2>사용자사전</h2>
	<div class="searchBox">
	<div class="box">
	<form action="userDic.jsp" method="post">
	...
	<%
		...
		if(result.size() > 0){
			...
			for(int i = 0; i < limit; i++){
				SetDictionaryVO sd = (SetDictionaryVO) result.get(i);
	%>
		<tr>
	...
	<div class="list_bottom">
		...
		<div style="width:100%">
			<span style="float:left">
				<input type="button" value="선택삭제" onclick="javascript:deleteCustomWord();" class="btn_del" style="float:left"/>
				<input type="button" value="초기화" class="btn_init" onclick="clearUserDict();" style="float:left"/>
			</span>
			<p class="addbox">
				<input type="text" class="inp02" tabindex="1" id="customword" size="20" onkeydown="javascript:addCustomWord(event);" maxlength="20" title="입력단어의 길이는 20자이내입니다."/>
				<input type="button" value="새로추가" onclick="javascript:addCustomWord();" class="btn_c"/>
			</p>
			<p class="clear"></p>
		...
		<form action="dbservice.jsp" method="post" id="addwordForm">
			<input type="hidden" name="customwordReal" id="customwordReal"/>
			<input type="hidden" name="category" value="<%=category %>"/>
