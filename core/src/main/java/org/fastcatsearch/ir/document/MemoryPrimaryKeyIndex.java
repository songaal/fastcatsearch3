 ...
 *
 */
public class MemoryPrimaryKeyIndex implements BytesToIntWriter, BytesToIntReader {
	...
	public MemoryPrimaryKeyIndex(int bucketSize) throws IOException{
		this(0, bucketSize);
	}
	public MemoryPrimaryKeyIndex(int indexInterval, int bucketSize) throws IOException{
...
//		logger.debug("length="+length);
	...
	public void writeTo(IndexOutput output, IndexOutput indexOutput) throws IOException{
...
//		indexOutput.seek(p);
	...
	public long wokingMemorySize() {
		int size = 0;
		size += bucket.length * 4;
		size += keyUseSize;
		size += count * 12; //keyPos(4), nextIdx(4), intValueArray(4)
		size += 150; //객체 5개 * 30bytes
		return size;
	}
