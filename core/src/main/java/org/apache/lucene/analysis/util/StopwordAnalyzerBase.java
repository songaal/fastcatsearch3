...
public abstract class StopwordAnalyzerBase extends Analyzer {
	...
	/**
	 * Returns the analyzer's stopword set or an empty set if the analyzer has
	 * no stopwords
	 * 
	 * @return the analyzer's stopword set or an empty set if the analyzer has
	 *         no stopwords
	 */
	 ...
	 */
	protected static CharArraySet loadStopwordSet(final boolean ignoreCase, final Class<? extends Analyzer> aClass, final String resource, final String comment)
			throws IOException {
		Reader reader = null;
		try {
