...
public class MasterCollectionPostDocumentJob extends MasterNodeJob {
	...
	public JobResult doRun() throws FastcatSearchException {
        ...
        CollectionPostDocumentJob collectionPostDocumentJob = new CollectionPostDocumentJob();
        collectionPostDocumentJob.setArgs(new String[]{collectionId, documents});
        logger.info("Request add indexing job to index node[{}] >> {}", indexNodeId, indexNode);
		ResultFuture jobResult = nodeService.sendRequest(indexNode, collectionPostDocumentJob);
		if (jobResult != null) {
