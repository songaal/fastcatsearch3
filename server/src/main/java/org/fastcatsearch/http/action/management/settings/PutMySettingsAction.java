...
public class PutMySettingsAction extends AuthAction {
	...
	public void doAuthAction(ActionRequest request, ActionResponse response)
		...
		try {
			...
			if(sessionInfo != null) {
				UserAccountVO vo = mapper.getEntryByUserIdAndPassword(sessionInfo.getUserId(), password);
		...
		if(updated) {
			responseWriter.object().key("success").value("true")
			.key("status").value(1).endObject();
		} else {
			responseWriter.object().key("success").value("false")
			.key("status").value(1).key("message").value(message).endObject();
