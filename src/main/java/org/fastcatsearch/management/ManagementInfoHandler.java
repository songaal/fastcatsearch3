...
public class ManagementInfoHandler {
	...
	public void checkJvmMemoryInfo(JvmMemoryInfo jvmMemoryInfo){
		jvmMemoryInfo.init(
				(int)(memoryMXBean.getHeapMemoryUsage().getMax() / MEGABITE_UNIT)
				, (int)(memoryMXBean.getHeapMemoryUsage().getCommitted() / MEGABITE_UNIT)
				, (int)(memoryMXBean.getHeapMemoryUsage().getUsed() / MEGABITE_UNIT)
				, (int)(memoryMXBean.getNonHeapMemoryUsage().getMax() / MEGABITE_UNIT)
				, (int)(memoryMXBean.getNonHeapMemoryUsage().getCommitted() / MEGABITE_UNIT)
	...
	public void checkJvmCpuInfo(JvmCpuInfo jvmCpuInfo){
		...
		/*
		 * 2. LoadAvg 생성
		 * LoadAvg는 벤더에 독립적이며  1.6이상 버전에만 의존한다.
		 * */
