...
package org.fastcatsearch.summary;
		...
		if(useSummary){
			if(useHighlight && highlightTags != null){
				if(target.length <= summaryInfo.summarySize() || summaryInfo.summarySize() <= 0){
					...
					return highlightAndSummarize(target, summaryInfo.termList(), summaryInfo.orgList(), summaryInfo.summarySize(), highlightTags, true);
				}
			}
			if(target.length <= summaryInfo.summarySize() || summaryInfo.summarySize() <= 0){
				return target;
...
//						logger.debug("@@@@ occu ==> "+pos);
	...
	//
	private char[] highlightAndSummarize(char[] target, List<String> terms, List<String> orgTerms, int summarySize, String[] highlightTags, boolean useHighlight) {
//		if(terms.size() == 0)
//			return target;
